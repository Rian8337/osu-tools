    ...
    internal class OsuObjectInspectorDrawable : PoolableDrawableWithLifetime<OsuObjectInspectorLifetimeEntry>
    {
        ...
        private void refresh()
        {
            ...
            if (entry.DifficultyHitObject is not null)
            {
                ...
                panel.AddParagraph($"Tap Difficulty: {TapEvaluator.EvaluateDifficultyOf(entry.DifficultyHitObject, 50):N3}"); // OD10, not possible to get difficulty stats from here?
                panel.AddParagraph($"Rhythm Difficulty: {RhythmEvaluator.EvaluateDifficultyOf(entry.DifficultyHitObject, 50):N3}");
                panel.AddParagraph($"Flashlight Difficulty: {FlashlightEvaluator.EvaluateDifficultyOf(entry.DifficultyHitObject, false):N3}");
                panel.AddParagraph($"Visual Difficulty: {VisualEvaluator.EvaluateDifficultyOf(entry.DifficultyHitObject, false):N3}");
